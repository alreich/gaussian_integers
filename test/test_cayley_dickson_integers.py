...
class TestZi(TestCase):
    ...
    def test_parse_quaternion_string(self):
        ...
        self.assertEqual(Zi.parse_hypercomplex_string('1+2i+3j+4k+5L+6I+7J+8K'), [1, 2, 3, 4, 5, 6, 7, 8])
        self.assertEqual(Zi.parse_hypercomplex_string('2i+3j+4k-1.07+6I+7J+8K'), [-1.07, 2, 3, 4, 0, 6, 7, 8])
        self.assertEqual(Zi.parse_hypercomplex_string('  -1 +3j + 4k+5L +8K - 7J   '), [-1, 0, 3, 4, 5, 0, -7, 8])
        self.assertEqual(Zi.parse_hypercomplex_string('-5L+8K'), [0, 0, 0, 0, -5, 0, 0, 8])
        self.assertEqual(Zi.parse_hypercomplex_string('8.743e-37K'), [0, 0, 0, 0, 0, 0, 0, 8.743e-37])
        self.assertEqual(Zi.parse_hypercomplex_string('1+2i'), [1, 2])
        self.assertEqual(Zi.parse_hypercomplex_string('1+2i-3j-4k'), [1, 2, -3, -4])
        self.assertEqual(Zi.parse_hypercomplex_string('1+2i-3j-4k-5L+6I-7J+8K'), [1, 2, -3, -4, -5, 6, -7, 8])
        self.assertEqual(Zi.parse_hypercomplex_string('1-3k'), [1, 0, 0, -3])
        self.assertEqual(Zi.parse_hypercomplex_string('2i-4k'), [0, 2, 0, -4])
        self.assertEqual(Zi.parse_hypercomplex_string('2j-8K'), [0, 0, 2, 0, 0, 0, 0, -8])
        self.assertEqual(Zi.parse_hypercomplex_string('i+j-k'), [0, 1, 1, -1])
        self.assertEqual(Zi.parse_hypercomplex_string('j+L'), [0, 0, 1, 0, 1, 0, 0, 0])
        self.assertEqual(Zi.parse_hypercomplex_string('2i-1+3k-4j'), [-1, 2, -4, 3])
        self.assertEqual(Zi.parse_hypercomplex_string('L-k'), [0, 0, 0, -1, 1, 0, 0, 0])
        self.assertEqual(Zi.parse_hypercomplex_string('7-2.4e-3i+3.75j-4.0k'), [7, -2.4e-3, 3.75, -4.0])